...
def finetune_bert(args, train_texts, train_labels, val_texts, val_labels, test_texts, test_labels, seed):
    ...
    train_texts, train_labels, lm_texts, lm_labels = add_lm_to_texts(train_texts, train_labels, desired_train_noise_label, num_train_noisy_samps, seed=seed)
    ...
    if(model_name == "EleutherAI/gpt-neo-125M" or model_name == "gpt2-medium" or model_name == "openai-community/gpt2" or model_name == "EleutherAI/pythia-160M"):
    ...
    if(single_layer_analysis):
        ...
        for layer_idx in range(12):
            ...
            for epoch in range(epochs):
                ...
                print(f"Testing Loss: {val_loss:.4f}, Accuracy: {test_acc:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}")
        ...
        for layers_type, layer_idx_list in layers_mapping.items():
            ...
            for epoch in range(epochs):
                ...
                # Testing phase
                test_loss, test_acc, test_precision, test_recall, test_f1 = evaluate_model(model, test_loader, device)
                print(f"Test Loss: {val_loss:.4f}, Accuracy: {test_acc:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}")
            ...
            # train_acc_list_layers[layers_type] = train_acc_list
            # val_acc_list_layers[layers_type] = val_acc_list
            # test_acc_list_layers[layers_type] = test_acc_list
            # lm_acc_list_layers[layers_type] = lm_acc_list

            # train_loss_list_layers[layers_type] = train_loss_list
            # val_loss_list_layers[layers_type] = val_loss_list
            # test_loss_list_layers[layers_type] = test_loss_list
            # lm_loss_list_layers[layers_type] = lm_loss_list
...
if __name__ == "__main__":
    ...
    for seed in seeds_list:
        ...
        finetune_bert(args, np.array(train_df['text']), np.array(train_df['label']), \
